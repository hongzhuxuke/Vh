...
void WebRtcLiveWdg::Notice(const CSTDMapParam &mapParam) {
   ...
   if (iter != mapParam.end()) {
      ...
      if (baseInfo.mWebinarInfo.type == CLASS_STATUS_IN_CLASS && baseInfo.mWebinarInfo.layout != CLASS_LAYOUT_ONLY_VIDEO) {
...
{
   //if (page_Doc == index && JOIN_ROLE_Teacher == globalToolManager->GetClassSDK()->GetClassRoomBaseInfo().mUserInfo.role_type) {
   //   AddDocWebView();
   //}
   //else {
   //   deleteDocWebView();
   ...
   //if (page_Whiteboard == index && JOIN_ROLE_Teacher == globalToolManager->GetClassSDK()->GetClassRoomBaseInfo().mUserInfo.role_type) {
   //   AddWhiteBoradWebView();
   //}
   //else {
   //   deleteWhiteBoradWebView();
   //}
...
void WebRtcLiveWdg::deleteDocWebView()
{
   if (mDocCefViewWdg && JOIN_ROLE_Teacher == globalToolManager->GetClassSDK()->GetClassRoomBaseInfo().mUserInfo.role_type) {
      delete mDocCefViewWdg;
      mDocCefViewWdg = nullptr;
   }

}
void WebRtcLiveWdg::deleteWhiteBoradWebView()
{
   if (mWhiteBoardCefViewWdg && JOIN_ROLE_Teacher ==  globalToolManager->GetClassSDK()->GetClassRoomBaseInfo().mUserInfo.role_type) {
      delete mWhiteBoardCefViewWdg;
      mWhiteBoardCefViewWdg = nullptr;
   }
}
...
void WebRtcLiveWdg::AddWhiteBoradWebView() {
   ...
   if(nullptr == mWhiteBoardCefViewWdg)
...
void WebRtcLiveWdg::slotChangeHandsUpState(const int& iState)
{
   ...
   if (!mMemberListWdg->isVisible() && eHandsUpState_existence == iState && nullptr != mpPaintLable && JOIN_ROLE_Teacher == baseInfo.mUserInfo.role_type)
   {
   ...
   if (event) {
      ...
      case CustomEvent_FloatType: {
         ...
         if (mMultiClassFloatWnd && mTeacherRender) {
            ...
            mMultiClassFloatWnd->show(msgEvent->mbState);
            ...
            else
            {
               ...
               mMultiClassFloatWnd->move(rect.x() + (rect.width() - mMultiClassFloatWnd->width()) - 10, 10);
            }
         ...
         if (msgEvent)
         {
            ...
            if (mFloatWnd) {
               ...
               if (Render) {
                  ...
                  mFloatWnd->show(msgEvent->mbState);
                  ...
                  else
                  {
                     ...
                     mFloatWnd->move(rect.x() + (rect.width() - mFloatWnd->width()) - 10, 10);
                     //mFloatWnd->move(0,0);
                  }
...
void WebRtcLiveWdg::handleAVCapture(QEventStream* msgEvent)
{
   ...
   if (join_id == techerid && mTeacherRender) {
      ...
      mTeacherRender->SetStreamID(QString::fromStdString(msgEvent->strStream));
      if(mTeacherRender->mStrBigScreenId.compare(join_id) == 0)
         mTeacherRender->SetPageIndex(msgEvent->mbHasVideo == true ? TeacherRenderWdg_Page_Index_render : TeacherRenderWdg_Page_Index_NoCamera);
      else
         mTeacherRender->SetMultiCameraState(join_id, msgEvent->mbHasVideo == true);
   ...
   if (msgEvent->mStreamType <= VHStreamType_AVCapture) {
      ...
      if (msgEvent->mIVHCapture != VHCapture_OK) {
         ...
         if (nRet != VhallLive_OK) {
            ...
            if (mTeacherRender && baseInfo.mUserInfo.role_type == JOIN_ROLE_Teacher) {//讲师
               ChangePublishState(true);
               mTeacherRender->SetSmallRenderMicState(!baseInfo.mutingAll);
               mTeacherRender->SetMicState(msgEvent->mbHasAudio);//true 打开音频
			   
               if (msgEvent->mbHasAudio) {
				      mTeacherRender->OpenMic();
			      }
			      else {
				      mTeacherRender->CloseMic();
			      }
            
               mTeacherRender->SetCameraState(msgEvent->mbHasVideo );
               mTeacherRender->SetRoleType(JOIN_ROLE_Teacher);
               if(mTeacherRender->GetCameraState())
                  mTeacherRender->SetPageIndex(msgEvent->mbHasVideo == true ? TeacherRenderWdg_Page_Index_render : TeacherRenderWdg_Page_Index_NoCamera);
               ...
               if (baseInfo.mWebinarInfo.course_type == CLASS_TYPE_SMALL || 
                  ...
                  if (msgEvent->mbHasVideo) {
                     ...
                     //if(true && mTeacherRender->GetCameraState())
                        mTeacherRender->SetShowSmallRender(true );
                     //mTeacherRender->SetSmallRenderCameraState(msgEvent->mbHasVideo, true);
                     //mTeacherRender->SetSmallRenderCameraState(msgEvent->mbHasVideo&& mTeacherRender->GetCameraState(), true);
                     mTeacherRender->setTeacherSmallRenderCameraState(msgEvent->mbHasVideo&& mTeacherRender->GetCameraState());
                     VHStreamType iStreamType = globalToolManager->GetClassSDK()->CalcStreamType(msgEvent->mbHasAudio, msgEvent->mbHasVideo);
               ...
               if (nullptr != mRenderManager){
                std::shared_ptr<vhall::VideoRenderReceiveInterface> Receiver = mRenderManager->GetRenderWnd(QString::fromStdString(baseInfo.mUserInfo.join_id));
                  ...
                  if (baseInfo.mWebinarInfo.course_type == CLASS_TYPE_MULTI
                     /*&& JOIN_ROLE_Student == baseInfo.mUserInfo.role_type*/
                     && mTeacherRender->mStrBigScreenId.compare( QString::fromStdString( baseInfo.mUserInfo.join_id ) ) == 0)
                  {
                     globalToolManager->GetClassSDK()->StartRenderLocalStreamByInterface(iStreamType, mTeacherRender->GetRenderWnd());
                  }
                  else {
...
void WebRtcLiveWdg::CreateScreenShareWgd()
{
   if (NULL == m_pScreenShareToolWgd)
   {
      ...
      if (m_pScreenShareToolWgd) {
         ...
         connect(m_pScreenShareToolWgd, &CScreenShareToolWgd::sigOpMediaType, this, &WebRtcLiveWdg::slot_OpMediaType);
         connect(m_pScreenShareToolWgd, &CScreenShareToolWgd::sigCameraClicked, this, &WebRtcLiveWdg::slot_OnClickedCamera);

         if (mTeacherRender && JOIN_ROLE_Teacher == globalToolManager->GetClassSDK()->GetClassRoomBaseInfo().mUserInfo.role_type) {
            //connect(m_pScreenShareToolWgd, &CScreenShareToolWgd::sigCameraClicked, mTeacherRender, &TeacherRenderWdg::slot_OnClickedCamera);
            
            connect(m_pScreenShareToolWgd, &CScreenShareToolWgd::sigMicClicked, mTeacherRender, &TeacherRenderWdg::slot_OnClickedMic);
            connect(mTeacherRender, &TeacherRenderWdg::sig_cameraStateChange, m_pScreenShareToolWgd, &CScreenShareToolWgd::slotcameraStateChange);
         }
         else {
         }
...
void WebRtcLiveWdg::slot_OnClickedCamera(bool bOpen)
{
   if (bOpen)//显示
   {
      if (eMultiClassFloatType_Multi == mFloatType) {
         mMultiClassFloatWnd->show(false);
      }
      else if (eMultiClassFloatType_Float == mFloatType) {
         mFloatWnd->show(false);
      }

      mFloatType = eMultiClassFloatType_Null;
   }
   else//隐藏
   {
      if (mMultiClassFloatWnd && mMultiClassFloatWnd->isVisible()) {
         mMultiClassFloatWnd->hide();
         mFloatType = eMultiClassFloatType_Multi;
      }
      else if (mFloatWnd && mFloatWnd->isVisible()) {
         mFloatWnd->hide();
         mFloatType = eMultiClassFloatType_Float;
      }

   }
   //mMultiClassFloatWnd
   //m_pScreenShareToolWgd
}
...
void WebRtcLiveWdg::HandleGetDesktopStream(QEventStream* msgEvent)
{
   if (VHCapture::VHCapture_OK == msgEvent->mIVHCapture)
   {
      ...
      if (m_pScreenShareToolWgd) {
         ...
         ClassRoomBaseInfo baseInfo = globalToolManager->GetClassSDK()->GetClassRoomBaseInfo();
         if (baseInfo.mUserInfo.role_type == JOIN_ROLE_Teacher) {
            mTeacherRender->slot_OnClickedFloatRender(false);

            m_pScreenShareToolWgd->slotcameraStateChange(mTeacherRender->GetCameraState());
            m_pScreenShareToolWgd->slotMicStateChange(mTeacherRender->MicIsOpen());
         }
         else {
...
void WebRtcLiveWdg::slot_BtnStopScreenClicked()
{
   ...
   ClassRoomBaseInfo baseInfo = globalToolManager->GetClassSDK()->GetClassRoomBaseInfo();
   if (baseInfo.mUserInfo.role_type == JOIN_ROLE_Teacher) {      
      slot_closeFloat( QString::fromStdString( baseInfo.mUserInfo.join_id ));
      if (mMultiClassFloatWnd /*&& mMultiClassFloatWnd->isVisible()*/) {
         // mMultiClassFloatWnd->hide();
...
void  WebRtcLiveWdg::FadeOutTip(QString strMsg, int bTostIndex, bool bLongTimeShow)
{
   ...
   else if (this->isVisible()/* && this->isActiveWindow()*/)// 显示后就隐藏
   {
      ...
      for (int i = 0; i < 3; i++) {
         ...
         else if (mpFadeOutTipWidget[i]->isVisible() && mpFadeOutTipWidget[i]->DisplayMsg().compare(str) == 0) {
            iFindeObj = i;
            break;
         }
...
void WebRtcLiveWdg::HandleWebRtcRoomConnetSuc(QEvent* event) {
   ...
   if (mbFirstConnectWebRtcServer && globalToolManager->GetClassSDK()->GetCourseInfo().webinar_status == CLASS_STATUS_IN_CLASS && baseInfo.mUserInfo.role_type == JOIN_ROLE_Teacher) {
      if (globalToolManager->GetClassSDK()->HasVideoOrAudioDev()) {
         //有音视频设备则可以上麦
         globalToolManager->PostEventToSelectWnd(WND_ID_WEBRTC_MAIN, new CustomEventType(CustomEvent_HandleStartLocalCapture, QString::number(1), true));
      }
   }
         ...
         if (globalToolManager) {
            ...
            if (RoomInterface)
            {
               RoomInterface->SetBigScreen(baseInfo.mUserInfo.join_id, [&](const std::string& user, int code, const std::string& msg) {
                  ...
                  if (code != 0 && code != 200) {
                     globalToolManager->PostEventToSelectWnd(WND_ID_WEBRTC_MAIN, new CustomEventType(CustomEvent_ShowFadOutTipMsg, QString::fromStdString(msg), true));
                     return;
                  }
                  globalToolManager->PostEventToSelectWnd(WND_ID_WEBRTC_MAIN, new CustomEventType(CustomEvent_ChangeBigScreen, QString::fromStdString(user), true));
   ...
   if (msg) {
      if (msg->mRespCode != 200) {
         ...
         if (msg->mUserId.toStdString() == globalToolManager->GetClassSDK()->GetClassRoomBaseInfo().mUserInfo.join_id) {
            ...
            bool bState = msg->mUserData.toInt() == 1 ? false : true;
               if (mTeacherRender) {    
                  mTeacherRender->SetMicState(bState);  
               }  
               if (m_pScreenShareToolWgd && m_pScreenShareToolWgd->isVisible()) {
                  m_pScreenShareToolWgd->slotMicStateChange(bState);
               }
            }
            else {
               if (globalToolManager->GetClassSDK()->GetClassRoomBaseInfo().mWebinarInfo.course_type == CLASS_TYPE_MULTI) {
...
void WebRtcLiveWdg::HandleMuteVideo(QEvent* event) {
   ...
   if (msg) {
      if (msg->mRespCode != 200) {
         ...
         if (msg->mUserId.toStdString() == baseInfo.mUserInfo.join_id && event->type() == CustomEvent_RecvMuteVideo) {
            ...
            if (msg->mUserData.toInt() == 1) {
               ...
               if (globalToolManager->GetClassSDK()->CloseCamera()){
                  ...
                  if (baseInfo.mWebinarInfo.course_type == CLASS_TYPE_MULTI 
                     && mTeacherRender 
                     && baseInfo.mUserInfo.role_type == JOIN_ROLE_Student
                     && mTeacherRender->GetUserJoinID().trimmed().toStdString() == msg->mUserId.toStdString()) {
                     mTeacherRender->SetCameraState(false);
                  }
               }
         ...
         if (msg->mUserId.toStdString() == baseInfo.mUserInfo.join_id && event->type() == CustomEvent_MuteVideo) {//收到自己
            if (msg->mUserData.toInt() == 1) {
               FadeOutTip(Str_MuteCamera, Tost_Warning);
               if (nullptr != mRenderManager) {
                  mRenderManager->SetCameraQss(QString::fromStdString(baseInfo.mUserInfo.join_id), false);
               }
               if (baseInfo.mWebinarInfo.course_type == CLASS_TYPE_MULTI && mTeacherRender 
                  && baseInfo.mUserInfo.role_type == JOIN_ROLE_Student
                  && mTeacherRender->GetUserJoinID().trimmed().toStdString() == msg->mUserId.toStdString()) {
                  mTeacherRender->SetCameraState(false);
               }
...
void WebRtcLiveWdg::HandleLocalStreamIdChanged(QEvent *event) {
   ...
   if (msg) {
      ...
      if (msg->mProcessType <= VHStreamType_AVCapture) {
         ...
         else if (baseInfo.mWebinarInfo.course_type == CLASS_TYPE_MULTI) {//公开课
            if (baseInfo.mUserInfo.role_type == JOIN_ROLE_Teacher) {//当前用户是讲师
               if (mTeacherRender) {
                  mTeacherRender->SetStreamID(mLocalStream);
                  HWND id = mTeacherRender->GetRenderWndID();
               }
...
void WebRtcLiveWdg::slot_OpMediaType(const int& iOptype, const QPoint& posShow)
{
   if (nullptr != mLeftToolBarWdg) {
      switch (iOptype)
      {
      case  ShortcutWdg::OpMediaType_Testpaper: mLeftToolBarWdg->slot_InteractionItemClicked(LeftToolBarWdg::InterActionOpType_TESTPAPER); break;
      case  ShortcutWdg::OpMediaType_Signin: mLeftToolBarWdg->slot_InteractionItemClicked(LeftToolBarWdg::InterActionOpType_SIGNIN); break;
      case  ShortcutWdg::OpMediaType_answer: mLeftToolBarWdg->slot_InteractionItemClicked(LeftToolBarWdg::InterActionOpType_ANSWER); break;
      case  ShortcutWdg::OpMediaType_time: mLeftToolBarWdg->slot_InteractionItemClicked(LeftToolBarWdg::InterActionOpType_TIMER); break;
      default:break;
      }
   }
}
