...
public:
    ...
    inline void  GetHexListPtr(CTSTR lpName, unsigned char *c, int size) {
       if (size <= 0) {
          return;
       }

       List<DWORD> HexList;
       GetHexList(lpName,HexList);

       for (UINT i = 0; i<HexList.Num()&&i<UINT(size)&&size>0; i++) {
          c[i] = (unsigned char )HexList[i];
       }
    }
    ...
    void  SetHexListPtr(CTSTR lpName,unsigned char *,int);
