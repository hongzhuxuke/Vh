...
public:
   ...
   */
   ...
   //开始直播时，页面返回的信息
   virtual void SetStartLiveUrl(const StartLiveUrl& liveUrl) = 0;
   virtual void SetStartLiveParam(const QString streamToken, const QString streamName, const QString msgToken, bool hidelogo) = 0;
   virtual void GetStartLiveUrl(StartLiveUrl& liveUrl) = 0;

   virtual void ParamToInitApiResp(const QString& respMsg) = 0;
   virtual void GetInitApiResp(ClientApiInitResp& initApiResp) = 0;
   // 1 直播， 2 互动。 本地应用选择的活动类型。
   virtual void SetSelectLiveType(int liveType) = 0;

   virtual void ParamToGetVSSRoomBaseInfo(const QString& respMsg) = 0;
   virtual void GetVSSRoomBaseInfo(VSSGetRoomInfo & roomInfo) = 0;

   virtual void ParamToVSSRoomAttributes(const QString& respMsg) = 0;
   virtual void GetVSSRoomAttributes(VSSRoomAttributes & Attributes) = 0;

   virtual void ParamToVSSRoomPushUrl(const QString& respMsg) = 0;
   virtual void GetVSSRoomPushUrl(VSSRoomPushUrl & pushinfo) = 0;

   virtual void SetPrivateStartParam(const QString& privateStartInfo) = 0;
   virtual QString GetPrivateStartParam() = 0;

   virtual void SetJoinActiveFromLoginPage(bool isLoginPage) = 0;
   virtual bool GetJoinActiveState() = 0;

   virtual void SetMainUiLoginObj(QObject* obj) = 0;
   virtual QObject* MainUiLoginObj() = 0;

   virtual void ParamToLineParam(const QString& param, UserOnLineParam& lineParam) = 0;
};
