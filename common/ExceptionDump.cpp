...
void CExceptionDump::Init() {
   ...
   SetUnhandledExceptionFilter((LPTOP_LEVEL_EXCEPTION_FILTER)UnhandledExceptionFilterEx);
}
...
LONG WINAPI UnhandledExceptionFilterEx(struct _EXCEPTION_POINTERS* ExceptionInfo) {
   ...
   swprintf_s(lwzFileName, 128, L"%s_%s_%s_%s.dmp", GetAppDataPath().c_str(), curTime.toStdWString().c_str(), gCurClientType.c_str(), logIdBase.toStdWString().c_str());
   //创建dump文件
   HANDLE hDumpFile = CreateFile(lwzFileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
   if (NULL == hDumpFile) {
